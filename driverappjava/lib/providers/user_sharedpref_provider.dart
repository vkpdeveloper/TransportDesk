...
class UserPreferences with ChangeNotifier {
  ...
  void init() async {
    ...
    if (result == ConnectivityResult.mobile || result == ConnectivityResult.wifi) {
      ...
      DocumentSnapshot userData =
          await Firestore.instance.collection('vendor').document(_user.phoneNumber).get();
      ...
      _referral = userData.data['referral'];
    }
    notifyListeners();
  }
  ...
  String get getReferral => _referral;

  void setIsFree(bool val) async {
    FirebaseUser _user = await _utils.getCurrentUser();
    Firestore.instance.collection('vendor').document(_user.phoneNumber).updateData({
      "isFree": val,
    });
    _isFree = val;
    notifyListeners();
